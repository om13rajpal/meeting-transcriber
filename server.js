...
app.post('/api/transcribe', (req, res) => {
  upload.single('file')(req, res, async (err) => {
    ...
    if (!DEEPGRAM_API_KEY) {
      ...
      return res.status(500).json({ error: 'Server is missing DEEPGRAM_API_KEY. Add it to .env and restart the server.' });
    }
    if (!req.file) {
    ...
    try {
      isVideo = await hasVideoStream(uploadedPath);
...
app.listen(PORT, () => {
  ...
  if (!DEEPGRAM_API_KEY) {
    console.warn('WARNING: DEEPGRAM_API_KEY is not set. Add it to a .env file (see .env.example).');
  }
});
